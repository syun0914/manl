...
async def skill(
    ...
    if bn == '급식' or bn == '생일 급식':
        thumbnail = tem.Thumbnail(
            'https://rawcdn.githack.com/syun0914/manl_thumbnail/ed539db632ae5f59b39370b096a2c203ee52d86d/summer_2023/meal.png',
            tem.Link('http://112.186.226.178:4082/st'),
            True,
            1920,
            1920,
        )
        ...
        mt = params['meal_time']
        match params['meal_time']:
            case '통합':
                d = await util.meal_unit(j.loads(params['date'])['date'])
            case '아침' | '점심' | '저녁':
                d = await util.meal(j.loads(params['date'])['date'], mt, False)
        return gtem(
            tem.basicCard(
                thumbnail,
                d['title'],
        ...
        thumbnail = tem.Thumbnail(
            "https://rawcdn.githack.com/syun0914/manl_thumbnail/e05fea172fd6864ae0797e5e9e078cb04c6a543f/spring_2023/timetable.png",
            tem.Link("http://112.186.226.178:4082/st"),
            True,
            2560,
            2560,
        ...
        d = await util.timetable("1-4", params["day"], "서일고등학교")
        return gtem(
            tem.basicCard(
                thumbnail, d["title"], d["timetable"], forwardable=True
            ),
            q_replies=[RETRY],
        )
...
{notice.content}""",
        ...
        #    )
        b = [
            tem.LCI(*t[:2], tem.Link(t[2]))
            for t in await util.school_board(
                "http://seoilgo.cnehs.kr", "60587", "0401", "seoilgo"
            )
        ]
        return tem.carousel(
            *[
                tem.listCard(title, b[i*5:i*5+5]) for i in range(4)
            ],
            k_type="listCard",
            q_replies=[REFRESH],
        )
        ...
        return gtem(
            tem.simpleImage(
                f'https://chart.apis.google.com/chart?cht=qr&chs=547x547&chl='
                + quote(params['qr_msg']),
...
async def admin(
    ...
    if bn == "Eval":
        ...
        try:
            ...
            return gtem(
                tem.simpleText(
                    f"📢 공지 사항 작성에 오류가 발생해 실패했어요."
                ),
                q_replies=[RETRY]
            )
        return gtem(
            tem.simpleText(f"📢 공지 사항 작성에 성공했어요."),
            tem.simpleText(
                "공지 사항 코드는 '"
        ...
        try:
            await db.execute(
                delete(Notice).where(
                    Notice.notice_code == params["notice_code"]
                )
            )
            res = "성공"
            await db.commit()
        ...
        try:
            ...
            msg = f'{TITLE}\n\nkey가 중복이어서 사용자 추가에 실패했어요.'
        else:
            msg = f'{TITLE}\n\n사용자 추가에 성공했어요.'
        return gtem(tem.simpleText(msg), q_replies=[RETRY])
        ...
        try:
            u = (
                await db.execute(
                    select(User).where(
                        and_(*[getattr(User, k).like(v) for k, v in q.items()])
                    )
                )
            ).first()[0]
            list_items = [
                ...
                tem.listCard(
                    ...
                    [
                        ...
                        tem.QReply(
                            ...
                            "block",
                            "사용자 제거",
                            "63b115fa2a784f093357cef2"
                        ),
                        tem.QReply(
                            "수정",
                            "block",
                            "사용자 정보 수정",
                            "63b12e292a784f093357cf9b"
                        ),
                    ],
                ),
                contexts=[
...
6. 어떠한 숫자도 맞지 않으면 '아웃(OUT)'이에요."""
        ...
        stmt = select(NumberBaseball).where(
            NumberBaseball.user_code == user_code
        )
        ...
        if "종료" in query:
            ...
            return gtem(
                tem.itemCard(
                    [
                        tem.ItemList(*t)
                        for t in [("정답", ud.answer), ("횟수", str(ud.count))]
                    ],
                    alignment="right",
                    head=TITLE,
            ...
            bt = util.num_baseball_check(query, ud.answer)
            count = ud.count + 1
