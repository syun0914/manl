...
class QReply:
    ...
    속성:
        ...
        messageText: action이 'message'일 때, 사용자의 발화로 내보낼 텍스트
        blockId: action이 'message'일 때, 호출할 블록의 ID
...
class ListCardItem:
    ...
    속성:
        ...
        action: 눌렀을 때 수행할 동작, 종류와 설명은 아래 [종류]와 같습니다.
            [종류]
            - 'block': 블록을 호출합니다.
            - 'message': 사용자의 발화를 실행합니다.
        blockId: action이 'block'일 때, 호출할 블록 ID
        messageText: action이 'message'일 때, 사용자의 발화로 내보낼 텍스트
...
class Button:
    ...
    속성:
        label: 적히는 문구
        action:
            눌렀을 때 수행할 동작, 종류와 설명은 아래 [종류]와 같습니다.
            [종류]
            - 'block': 블록을 호출합니다.
            - 'message': 사용자의 발화로를 실행합니다.
            - 'webLink': URL로 이동합니다.
            - 'phone': 전화를 겁니다.
            - 'share': 말풍선을 다른 사용자에게 공유합니다.
            - 'operator': 상담직원을 연결합니다.
        webLinkUrl:
            action이 'webLink'일 때, 버튼을 눌렀을 때 이동할 URL
        phoneNumber:
            action이 'phone'일 때, 버튼을 눌렀을 때 전화할 전화번호
        blockId:
            action이 'block'일 때, blockId인 블록을 호출합니다.
        messageText:
            action이 'message' 또는 'block'일 때, 발화로 내보낼 텍스트
        extra: 스킬 서버에 추가적으로 제공할 정보
    '''
    label: str
    action: str
...
class CarouselHeader:
    ...
    def __post_init__(self):
        ...
        self.thumbnail = thumbnail and del_empty(asdict(thumbnail))


class CH(CarouselHeader):
    ...
