...
from kakao_template.exceptions import LengthError, ExistError
...
def simpleText(
    text: str, buttons: list[Button] | None = None, forwardable: bool = False
) -> dict:
    """챗봇 템플릿 (텍스트)
...
def listCard(
    ...
    """
    if len(items) > 5:
        raise LengthError("아이템은 5개 이하여야 합니다.")
    if buttons and len(buttons) > 2:
        raise LengthError("버튼은 2개 이하여야 합니다.")
    return {
        "listCard": del_empty(
            {
                "header": {"title": title},
...
def carousel(
    *templates,
    ...
    if k_type == "listCard":
        if len(templates) > 5:
            raise LengthError("템플릿 타입이 'listCard'일 때, 템플릿은 5개 이하여야 합니다.")
        if header:
            raise ExistError("템플릿 타입이 'listCard'일 때, 캐로셀 헤더를 사용할 수 없습니다.")
    elif len(templates) > 10:
        raise LengthError("템플릿은 10개 이하여야 합니다.")
    return {
        "version": VERSION,
        "template": {
            "outputs": [
            ...
            "quickReplies": (q_replies and [del_empty(asdict(q)) for q in q_replies]),
        },
        "context": del_empty(
            {"values": contexts and [del_empty(asdict(c)) for c in contexts]}
        ),
...
def itemCard(
    ...
    """
    if head and profile:
        raise ExistError("head, profile을 같이 사용할 수 없습니다.")
    return {
        "itemCard": del_empty(
            {
                "thumbnail": thumbnail and del_empty(asdict(thumbnail)),
